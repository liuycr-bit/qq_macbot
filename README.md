...
| 本地表情扩展 | 已完成 | `#meme` 独立路由、Worker/子进程隔离、NapCat 登录态头像桥、消息图片输入和 902 个固定版本模板已在本机启用 |
...
- 增加 `meme-emoji`、官方 `meme-generator-contrib-rs`、QQ Agent 近期热梗包，以及用户选定的 B1/B2 模板源；固定版本在本机共加载 902 个模板，第三方程序和资源只写入应用数据目录。B3/C1 因不支持头像嵌入已移除。
...
头像桥安装脚本只写入 NapCat 的程序/数据目录，不修改 `QQ.app`。首次安装或 NapCat 更新后执行一次，并重启 QQ/NapCat 与 QQ Agent。表情生成器和 902 个模板的完整安装步骤见 [本地表情生成扩展](docs/MEME_EXTENSION.md)。
...
11. **方案参考**：[SodaSizzle/astrbot_plugin_meme_generator](https://github.com/SodaSizzle/astrbot_plugin_meme_generator)。本仓库的实现不依赖 AstrBot，也未把该插件源码复制进来。
